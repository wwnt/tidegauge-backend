...
func GetGlossData() (interface{}, error) {
	rows, err := TideDB.Query(`SELECT "@_id", name, sga.country, latitude, longitude, "latestPsmsl", "latestBodc", "latestSonel", "latestJasl", "latestUhslcFast", "latestVliz", "latestPsmslRlr", "IOC code", "PSMSL number" FROM stationinfo_gloss_all sga left join stationinfo_gloss sg on sga.name = sg."Station"`)
	if err != nil {
		return nil, err
	}
	var (
